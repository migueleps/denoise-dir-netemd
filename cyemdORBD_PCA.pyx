...
def dis2(s): #Same as dis but now allows count values in R instead of N, which is useful to use the reconstruction from projecting the data using ICA
...
def get_cum(queries,orb): #get cdfs for orbit orb
    return [cum(rescale(dis2(readcount(queries[i]+'.countsO',orb)))) for i in range(len(queries))]

def MKSA(queries,orb): #computes NetEmd between queries for orbit orb
    n=len(queries)
    C=get_cum(queries,orb)
    K=zeros((n,n))
    for i in range(n):
