...
def count(name):
        ...
        if os.path.isfile("5d_precomputed/"+n+".countsO5d"):
                shutil.copy2("5d_precomputed/"+n+".countsO5d",sdir+name+".countsO5d")
        else:
                if not os.path.isfile(name+'.mod'):
                        prepare_graph(name)
                while True:
                        os.system('gtscanner/GTScanner -s 5 -d -g '+name+'.mod -m gtrie gtscanner/gtries/or_dir5.gt -or -th {} -countsO {}.countsO5d'.format(n_threads,name))
                        if os.stat(name+".countsO5d").st_size > 0:
                                break
                        print("Failed to compute 5D orbits for {}, trying again".format(name))
                shutil.copy2(sdir+name+".countsO5d",sdir+"5d_precomputed/")
