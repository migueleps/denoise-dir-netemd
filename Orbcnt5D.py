...
def count(name):
        ...
        if os.path.isfile("5d_precomputed/"+n+".countsO5d"):
                ...
                if not os.path.isfile(name+'.mod'):
                        prepare_graph(name)
                while True:
                        os.system('gtscanner/GTScanner -s 5 -d -g '+name+'.mod -m gtrie gtscanner/gtries/or_dir5.gt -or -th 50 -countsO '+name+'.countsO5d')
                        if os.stat(name+".countsO5d").st_size > 0:
                                break
                        print "Failed to compute 5D orbits for %s, trying again" % name
