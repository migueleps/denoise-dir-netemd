...
def get_cum(queries,orb): #get cdfs for orbit orb
    return [cum(rescale(dis(readcount(queries[i]+'.countsO',orb)))) for i in range(len(queries))]

def orb_exists(queries,orb):
    n=len(queries)
    S=zeros((n,n))
    ss = [sum(readcount(queries[i]+'.countsO',orb)) for i in range(n)]
    for i in range(n):
