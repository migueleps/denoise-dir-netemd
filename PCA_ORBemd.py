...
#        cy.toM(K,queries,"PCA_"+outdir+"/NetEmd_Orb"+str(i)+indir)
        if i==0:
            ...
            Ms=Ms+K# * weights[i]
        if i==3:
            cy.toM(Ms/(i+1),queries,'NetEmd_G3_5W_{}_PCA{}'.format(indir,expvar))
        elif i==14:
            cy.toM(Ms/(i+1),queries,'NetEmd_G4_5W_{}_PCA{}'.format(indir,expvar))
        elif i==72:
            cy.toM(Ms/(i+1),queries,'NetEmd_G5_{}_PCA{}'.format(indir,expvar))
