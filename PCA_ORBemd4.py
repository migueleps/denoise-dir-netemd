...
def pca_rewrite(queries):
    for f in queries:
        raw_counts = pd.read_csv(f+".countsO",sep="\t",usecols=range(15))
...
#    weights = get_orbit_weights(queries)
    ...
    for i,K in enumerate(c):
        ...
        if i==0:
            ...
            Ms=Ms+K
        if i==3:
            cy.toM(Ms/(i+1),queries,'NetEmd_G3_4W_{}_PCA{}'.format(indir,expvar))
        elif i==14:
            cy.toM(Ms/(i+1),queries,'NetEmd_G4_{}_PCA{}'.format(indir,expvar))
