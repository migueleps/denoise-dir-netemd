...
void Isomorphism::canonicalStrNautyMatrix(char *in, char *s) {
  int i, j, aux;
...
/******* Graphlets *******/
int* Isomorphism::computeOrbits(const char *in, int size){
  ...
  nauty(g,lab,ptn,NULL,orbits,&options,&stats,
    workspace,WORKSPACE_SIZE,m,size,mm);

  int max_orbit = 0;

  /*printf("Orbits: [");
  for(int orbit = 0; orbit < size; orbit++){
    printf("%d ", orbits[orbit]);
  }
  printf("]   ||    ");*/
  ...
  for(int a = 0; a < size; a++){
    for(int orbit = 0; orbit < size; orbit++)
      if(orbits[orbit] == a)       found = true;
      else if(orbits[orbit] > max) max   = orbits[orbit];
    if(found == false && a < max){
      for(int orbit = 0; orbit < size; orbit++)
        if(orbits[orbit] > a) orbits[orbit]--;
      a = 0;
    }
    found = false;
    max   = 0;
  ...
  //printf("Orbits: [");
  for(int orbit = 0; orbit < size; orbit++){
    if(orbits[orbit] > max_orbit)
        max_orbit = orbits[orbit];
    orbits[orbit] += total_orbits;
    //printf("%d ", orbits[orbit]);
  }
  //printf("]\n");
  total_orbits += max_orbit + 1;
