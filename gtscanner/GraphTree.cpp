...
void GraphTreeNode::setString(int pos, char *s, long long f) {
...
void GraphTreeNode::addString(int pos, char *s, long long f) {
  if (s[pos]==0) frequency+=f;
  else {
    if (s[pos]=='1') {
      if (one==NULL) one = new GraphTreeNode();
...
bool GraphTreeNode::equal(GraphTreeNode *gt, int pos, char *s) {
  ...
  if (zero == NULL && one == NULL) {
    ...
    if (frequency != gt->frequency) 
      printf("NOT EQUAL TREE %s: %lld != %lld\n", s, frequency, gt->frequency);
...
bool GraphTreeNode::equal(GTrie *gt, int size, int pos, char *s) {
  if (zero == NULL && one == NULL) {
    ...
    if (frequency != aux)
      printf("NOT EQUAL GTRIE %s: %lld != %lld\n", s, frequency, aux);
...
void GraphTreeNode::populateMap(mapStringLL *m, int size, int pos, char *s) {
  if (zero == NULL && one == NULL) {
    s[pos]=0;
    char s2[size*size+1];
...
long long GraphTreeNode::countOccurrences() {
  long long aux = frequency;
...
void GraphTree::setString(char *s, long long f) {
  root->setString(0, s, f);
}

void GraphTree::addString(char *s, long long f) {
...
void GraphTree::populateMap(mapStringLL *m, int maxsize) {
  char s[maxsize*maxsize+1];  
...
long long GraphTree::countOccurrences() {
