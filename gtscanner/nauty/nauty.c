...
static __thread unsigned long invapplics,invsuccesses;
static __thread int invarsuclevel;

    /* working variables: <the "bsf leaf" is the leaf which is best guess so
                                far at the canonical leaf>  */
static __thread int gca_first,     /* level of greatest common ancestor of current
                                node and first leaf */
...
static __thread boolean needshortprune;       /* used to flag calls to shortprune */
...
static __thread tcnode tcnode0 = {NULL,NULL};
static __thread int alloc_m = 0;

#else
static __thread set defltwork[2*MAXM];        /* workspace in case none provided */
static __thread permutation workperm[MAXN];   /* various scratch uses */
static __thread set fixedpts[MAXM];           /* points which were explicitly
                                        fixed to get current node */
static __thread permutation firstlab[MAXN],   /* label from first leaf */
                   canonlab[MAXN];   /* label from bsf leaf */
static __thread short firstcode[MAXN+2],      /* codes for first leaf */
             canoncode[MAXN+2];      /* codes for bsf leaf */
static __thread shortish firsttc[MAXN+2];     /* index of target cell for left path */
static __thread set active[MAXM];             /* used to contain index to cells now
                                        active for refinement purposes */
#endif

static __thread set *workspace,*worktop;      /* first and just-after-last addresses of
                                        work area to hold automorphism data */
static __thread set *fmptr;                   /* pointer into workspace */
