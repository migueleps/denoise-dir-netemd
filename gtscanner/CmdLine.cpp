...
void CmdLine::run_gtrie(Graph *g, GraphTree *sg) {
  ...
  if(Global::compute_orbits){
	gt_original->writeGraphletCountToFile(g->numNodes(), countsO);
	//FILE *fo = fopen("insanity","w");
	//computeGDD(gt_original->orbit_frequency, g->numNodes(), gt_original->total_orbits, fo);
...
void CmdLine::compute_results() {
  int i, j;
  mapStringLL:: const_iterator ii, iiend;

  // Create map and init results
  mapStringLL m_original;  
  ...
  if (random_number > 0) {
    ...
    mapStringLL *m_count = new mapStringLL[random_number];
...
void CmdLine::show_results(ResultType *res, int nres) {
  ...
  for (i=0; i<nres; i++) {
    ...
    if (html) 
      fprintf(f_output, "<tr class=\"%s\"><td><img src=\"http://www.dcc.fc.up.pt/gtries/graph.php?%swidth=75&height=75&adj=%s\"></td><td class=\"pre\">%s</td><td>%lld</td><td>%.2f</td><td>%.2f +/- %.2f</td></tr>\n",
...
void CmdLine::parse_cmdargs(int argc, char **argv) {
  ...
  for (int i=1; i<argc; i++) {
    ...
    // MS
    else if (!strcmp("-countsO",argv[i]))
      countsO = fopen(argv[++i],"w");
    /**********************************/
  }
...
void CmdLine::compute_gda_dir(){
  ...
  float **gda_mat = new float*[gddFiles.size()];
  for (int i=0; i<gddFiles.size(); i++) {
    gda_mat[i] = new float[gddFiles.size()];
    gda_mat[i][i] = 1;
  }

  FILE *gda_file = fopen("gda_results.txt","w");

  for (int i = 0; i<gddFiles.size(); i++)
    fprintf(gda_file,"%s ",gddFiles[i].c_str()); 
  fprintf(gda_file,"\n");
	    

  /*bool **compute = new bool*[gddFiles.size()];
  for (int i=0; i<gddFiles.size(); i++) {
    compute[i] = new bool[gddFiles.size()];
    compute[i][i] = false;
}
    
  string delim = "_";
  for (int i=0; i<gddFiles.size(); i++) {
    string prefix = gddFiles[i].substr(gddFiles[i].find(delim)+1,gddFiles[i].substr(gddFiles[i].find(delim)+1,gddFiles[i].length()).find(delim));
    for (int j=i+1; j<gddFiles.size(); j++) {
      string jpre = gddFiles[j].substr(gddFiles[j].find(delim)+1,gddFiles[j].substr(gddFiles[j].find(delim)+1,gddFiles[j].length()).find(delim));;
      compute[i][j] = prefix != jpre;
      compute[j][i] = compute[i][j];
    }
    }*/
  
  float gda_val = 0;
  for(int i = 0; i < gddFiles.size(); i++){
	for(int j = 0; j < gddFiles.size(); j++){
	  /*if (!compute[i][j]) {
	    fprintf(gda_file,"0.0 ");
	    continue;
	    }*/
	  if (j <= i) {
	    fprintf(gda_file,"%.5f ",gda_mat[j][i]);
	    continue;
	  }
	  gda_val = gda.computeGDA(i, j);
	  gda_mat[i][j] = gda_val;
	  fprintf(gda_file,"%.5f ",gda_val);
